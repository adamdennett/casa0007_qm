...
## Text formatting{background-image="https://www.ucl.ac.uk/bartlett/sites/bartlett/files/styles/funnelback/public/muyuan-ma-FJ9jgG2Kp1U-unsplash.jpg.webp?itok=5dGaSs6X" background="#f9dd73" background-opacity=0.5}

The quote background is slightly transparent, you can still see the image through it!
...
ggplot(mtcars, aes(hp, mpg, color = am)) +
  ...
  casaviz::theme_casa(base_text_size = 16)
```
...
# Here's a section title slide {background-image="https://www.ucl.ac.uk/bartlett/sites/bartlett/files/styles/funnelback/public/american-public-power-association-eIBTh5DXW9w-unsplash.jpg.webp?itok=pLc-Wvid" background="#4e3c56" background-opacity=0.4}

You can change the image, and overlay a dark colour to make the text automatically go white.
...
ggplot(palmerpenguins::penguins, 
       aes(x = bill_length_mm, 
           ...
           color = species)) +
  geom_point() +
  casaviz::theme_casa(background = FALSE) +
  casaviz::scale_colour_casa()
```
...
::: {.column width="62%"}
```{r}
head(mtcars)[,c("mpg",	"cyl", "disp", "hp", "wt")] |> casaviz::casa_flextable()
```
:::
:::
...
## Slide Backgrounds {.half-and-half}

I also created a half-and-half slide format, for adding some accents
...
ggplot(mtcars, aes(hp, mpg, color = am)) +
  geom_point(size = 5) +  
  geom_smooth(formula = y ~ x, method = "loess",
              colour = casaviz::casa_colours$dark_green,
              fill = casaviz::casa_colours$blue, 
              alpha = 0.2) +
  casaviz::theme_casa(background = FALSE) +
  casaviz::scale_colour_casa("warm_colours", 
                            reverse = TRUE, 
                            continuous = TRUE)
```
...
```{r}
mtcars |> reactable::reactable(theme = casaviz::casa_reactable_theme(colour = "purple"))
```
:::
...
![](https://github.com/cararthompson/30DayChartChallenge/raw/main/plots/2.5_circular.png){.r-stretch}
