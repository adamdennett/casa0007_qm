...
-   Both derived from the sum of the squared residual error (SSE)
-   The F-Statistic is a ratio of the amount of variance explained by the model to the amount of variance not explained by the model - overall p-value <0.05 = the model is statistically significant
...
-   In our example, we have 10 observations and 2 parameters = 5:1 ratio = potentially unreliable model
...
-   The Estimates ($\beta$)
    -   Intercept - estimated Attainment 8 when % Disadvantaged Children in a school is zero
    -   PTFSM6CLA1A - effect of a one-unit change in the % Disadvantaged Children on Attainment 8 - therefore linked to the units of the independent variable and not directly comparable across different variables if measured on different scales
...
-   The p-values relate to the null hypotheses that:
    -   The true value of the intercept is zero when the independent variable is zero. P \<0.001 = \<0.1% probability that 62.3 occured by random chance
    ...
    -   The true value of the PTFSM6CLA1A slope (relationship between % Disadvantaged and Attainment 8) is 0. P \< 0.01 = \<1% probability that the relationship observed is by chance. Relationship is likely to be real.
...
-   It's also unclear whether there are any other factors that might be correlated with disadvantaged students that might also be influencing Attainment 8 scores and ***confounding*** the apparent relationship we observe
...
-   Negative association present - appears to confirm Gorard observation
...
-   Some suggestion of non-linearity with big up-tick at the low end of disadvantage 
...
-   A significant deviation from a normal distribution could mean that the p-values are not trustworthy
...
## Linear Regression - Dealing with Non-Linearity
...
-   The log() log() model is a standard transformation that can help to linearise relationships that are not linear

## Linear Regression - the Log Log transform
...
-   The log-log transformation has helped to linearise the relationship between % Disadvantaged Students and Attainment 8
-   The log-log model means that while we now have a linear model, the effect on Attainment 8 changes depending on the level of % Disadvantaged Students
-   We can see from our original plot that the effect of levels of disadvantage on attainment is much stronger at very low levels of disadvantage and weakens as the level of disadvantage increases
