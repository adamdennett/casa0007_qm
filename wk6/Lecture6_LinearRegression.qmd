...
```{mermaid}
flowchart TD
    I{Choosing a statistical test} -->
    A[How Many Variables?] -->
    C[2?] & G[More than 2?]
    C -->|Categorical?| D[Chi Squared or Similar]
    C -->|Scale or Ratio?| E[Pearson Correlation or Spearman's Rank]
    C -->|Both?| F[Ask Google]
    G --> H[REGRESSION - everything else, get in the bin]
```
...
-   The fit of the model is represented by the ***coefficient of determination*** or $R^2$ value, which is calculated from the residuals
...
-   The p-values relate to the ***null hypotheses*** that:
    1.  true value of the intercept is **zero** when the independent variable is **zero**. P \<0.001 = \<0.1% probability that 62.3 occured by random chance - It doesn't tell us that when % Disadvantaged is zero, 62.3 will be a correct Attainment 8 score, just real value is unlikely to be zero
...
-   They are instead displaying ***heteroscdasticity***
...
-   Despite some positive correlation (0.48) between % Disadvantaged Students and % Persistent Absence, the VIF is very low \~1.5
...
-   This is important because if the residuals are (auto)correlated, it suggests that there is some pattern in the errors that the model has not captured, which can lead to biased estimates and incorrect conclusions
-   (auto)correlated residuals can occur when there is a time or spatial component to the data, or when there are unobserved variables that are influencing the dependent variable
...
-   One of the nice features of a linear regression model is the independent variables don't just have to be on a continuous or ratio scale, but they can also be categorical / nominal
-   Categorical variables added to a regression model are referred to as ***DUMMY*** variables
-   In our model, we might want to add the names of the regions that the schools fall in as dummy variables to see if indeed the "London" effect suggested in the residual map is real
...
-   South East as contrast - London's log(ATT8SCR) is 0.0957 higher
...
![](images/confound1.png)

## Multiple Linear Regression - Confounding

![](images/confound2.png)
...
-   In the presence of persistent absence, the effect of disadvantage is confounded

## Multiple Linear Regression - Confounding

-   The coefficient for log(PTFSM6CLA1A) - disadvantage - shrinks dramatically, from -0.207 down to -0.111 (a 46.23% reduction in effect)
-   While the coefficient for log(PERCTOT) - persistent absence - also shrinks from -0.54 to -0.41 this is just a 24.6% reduction
-   The confounding effect of persistent absence on disadvantage is ***almost double*** the confounding effect of disadvantage on persistent absence
-   This means that the relationship between disadvantage and Attainment 8 is much more biased by the omission of persistent absence from the model than the other way around
...
-   Including a regional dummy ***slightly reduces the effect of persistent absence*** and ***slightly increases the effect of disadvantage***, relative to model 3
...
-   Part of the negative effect of persistent absence is due to its correleation with regional factors - but which ones?
-   We can explore this question through one final twist in our modelling recipe - we can look at the ***interaction effects*** between individual regions and disadvantage / persistent absence

## Multiple Linear Regression - Interaction Effects

-   Put simply, an interaction term allows the model to answer the question: ***"Does the effect of disadvantage/persistent absence on a school's Attainment 8 score change from region to region?"***
...
-   Main Effect (Reference Group):
    ...
    -   The coefficient for log(PERCTOT) is -0.289960. This is the effect of persistent absence in the South East.
-   Interaction Effects (Reference Group value + Other Region coefficient):
    ...
    -   gor_nameLondon:log(PERCTOT): The significant negative coefficient (-0.069792) means that the negative effect of persistent absence is more severe in London than in the South East.
